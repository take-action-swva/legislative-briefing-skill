...
Everything this skill has published to **Google Drive > Statewide Coordinating
Committee > Legislation Briefings**, newest first.

This file exists because `brief-short.md` is told to link to the full brief and
had nothing to look in. Searching Drive from a session is slow and the folder
name is easy to get wrong; this is the fast path.

**Update this whenever `./scripts/publish.sh` runs.** The script cannot do it
for you — it does not know which issue a file belongs to.
...
`fisa-702-brief (1).docx` is in the folder as an apparent duplicate of
`fisa-702-brief.docx` and is not listed here. Confirm which is current before
linking either.
